  ...
  group("Selector utils", () {
    ...
    group("ensureRecordPosition", () {
      test('when empty add the record in 1 and then in 2', () {
        Selector selector = Selector();
        final record1 = Record(info: simpleInfo1, isDouble: false);
        selector.ensureRecordPosition(record1);
        expect(record1.position, 1);
        selector.records.add(record1);

        final record2 = Record(info: simpleInfo2, isDouble: false);
      ...
      test('add in the next possible position', () {
        ...
        final record3 = Record(info: simpleInfo1, isDouble: false);
        record3.position = 4;
        selector.records.add(record3);

        final record = Record(info: simpleInfo1, isDouble: false);
      ...
      group("double vinyle", () {
        test('when empty add the record in 1 and then in 3', () {
          Selector selector = Selector();
          final record1 = Record(info: doubleInfo1, isDouble: true);
          selector.ensureRecordPosition(record1);
          expect(record1.position, 1);
          selector.records.add(record1);

          final record2 = Record(info: doubleInfo2, isDouble: true);
          selector.ensureRecordPosition(record2);
        ...
        test('add in the next possible position', () {
          ...
          final record = Record(info: doubleInfo1, isDouble: true);
          selector.ensureRecordPosition(record);
        ...
        test('add in the next possible even position with a gap', () {
          ...
          record3.position = 3;
          selector.records.add(record3);
          final record4 = Record(info: simpleInfo1, isDouble: false);
          record4.position = 5;
